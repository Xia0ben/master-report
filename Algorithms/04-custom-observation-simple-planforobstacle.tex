    ...
    \Procedure{PLAN-FOR-OBSTACLE}{$o$, $p_{opt}$, $I$, $R$, $R_{goal}$, $blockedObsL$}
      ...
      \For{each $pushPose$ in $o.pushPoses$}
        ...
        \If{\hlgreen{$o.movableStatus = IS\_MAYBE\_MOVABLE$}}
          ...
          \If{\hlgreen{$obsPose \neq$ null}}
            \State \hlgreen{$o_{1}, c_{1} \gets c_{1}[c_{1}.firstPose:obsPose], c_{1}[obsPose:c_{1}.lastPose]$} \Comment{}
          \Else
            \State \hlgreen{COMPUTE-O1-C1($o$, $I$, $R$, $pushPose$, $o_{1}$, $c_{1}$)}

            \If{\hlgreen{$o_{1} = \emptyset$ OR $c_{1} = \emptyset$}}
        ...
        \State $C_{est} \gets ($\hlgreen{$(o_{1} \neq \emptyset ? |o_{1}| : 0)$}$ + |c_{1}| + |c_{3_{(Est)}}|) * moveCost + |translation| * o.pushCost$
        ...
        \While{$C_{est}$ $ \leq p_{opt}.cost$ AND $safeSweptArea \neq$ null}
          ...
          % \If{CHECK-NEW-OPENING($I.occGrid$, $o$, $translation$, $BA$)}
            ...
            \If{$c_{3} \neq \emptyset$}
              \State $p.components \gets$ \hlgreen{$o_{1} \neq \emptyset$ ? [$o_{1}$, $c_{1}$, $c_{2}$, $c_{3}$] : [$c_{1}$, $c_{2}$, $c_{3}$]}
              \State $p.cost \gets ($\hlgreen{$(o_{1} \neq \emptyset ? |o_{1}| : 0)$}$ + |c_{1}| + |c_{3}|) * moveCost + |c_{2}| * o.pushCost$
