...
\usepackage[backend=bibtex,style=numeric,natbib=true,sorting=ydnt]{biblatex} % Use the bibtex backend with the authoryear citation style (which resembles APA)
...
% Custom adjustements
% Packages to display algorithms properly
\usepackage{algorithm}
\usepackage{algorithmicx}
\usepackage{algpseudocode}

% Package for easier placement of content
\usepackage{float}

% Packages for subfigures and associated captions
\usepackage{caption}
\usepackage{subcaption}

% Package needed for highlighting
...
% Package for inserting pdfs pages into the document
\usepackage{pdfpages}
%% Use \includepdf[pages={-},fitpaper,rotateoversize]{<filepath>.pdf} to include a pdf

% Package for inserting actual code into the document
\usepackage{minted}
...
% First title was: Toward social navigation among movable obstacles
\thesistitle{Analyzing robot Navigation Among Movable Obstacles in unknown environments to extend it to social and dynamic constraints} % Your thesis title, this is used in the title and abstract, print it elsewhere with \ttitle
\supervisor{Olivier \textsc{Simonin}\\
			Jacques \textsc{Saraydaryan}\\
			Fabrice \textsc{Jumel}\\} % Your supervisor's name, this is used in the title page, print it elsewhere with \supname
\examiner{Jean-François \textsc{Boulicaut}\\
			Vasile-Marian \textsc{Scuturici}\\
			Christine \textsc{Solnon}\\} % Your examiner's name, this is not currently used anywhere in the template, print it elsewhere with \examname
...
\keywords{NAMO, Navigation Among Movable Obstacles, Optimality, Social Navigation, Dynamic environments} % Keywords for your thesis, this is not currently used anywhere in the template, print it elsewhere with \keywordnames
\keywordsfr{NAMO, Navigation parmi des obstacles mobiles, Optimalité, Navigation Sociale, Environnement dynamique}
\university{\href{http://insa-lyon.fr}{INSA Lyon}} % Your university's name and URL, this is used in the title page and abstract, print it elsewhere with \univname
...
\textsc{\Large Detailed Research Report}\\[0.5cm] % Thesis type
...
\large \textit{A report written in the \groupname \\ and submitted in fulfillment of the requirements of the \degreename \\ of the \deptname}\\[0.3cm] % University
...
\input{Appendices/comparison_tables.tex}
